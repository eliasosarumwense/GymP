            ...
            ZStack {
                ...
                .chartXAxis {
                    AxisMarks(values: xAxisValues) { value in
                        ...
                        AxisValueLabel() {
                            if let date = value.as(Date.self) {
                                Text(dateFormatterForSelectedDate.string(from: date))
                                    .customFont(.light, 10)
                                    .foregroundStyle(Color.gray)
                            }
                        }
                    ...
                    Rectangle()
                        ...
                        .overlay {
                            HStack {
                                if showFullWeight {
                                    VStack {
                                        Text("\(getFullWeight(selectedWeight, selectedReps) ?? "") kg")
                                    ...
                                    VStack {
                                        Text("\(selectedReps) x \(selectedWeight) kg")
                                            .font(.customFont(.semiBold, 13))
                                            .foregroundColor(.white)
                                            .onChange(of: selectedWeight) {
                                                triggerHapticFeedbackRigid()
