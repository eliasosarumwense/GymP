...
struct CalendarView: View {
        ...
        VStack(spacing: rowSpacing) {
            ...
            ZStack {
                ...
                if let nextMonth = calendar.date(byAdding: .month, value: -1, to: currentMonth) {
                    let isNextMonthDisplayed = calendar.isDate(currentDate, equalTo: nextMonth, toGranularity: .month)
                    
                    if isCurrentMonthDisplayed || isNextMonthDisplayed {
                                        Rectangle()
    ...
    private func generateMonthDates() -> [DateData] {
        ...
        if paddingDaysAfter > 0 {
            ...
            // Step 2: Add one month to get the start of the next month
            guard let nextMonthStart = calendar.date(byAdding: .month, value: 1, to: firstDayOfCurrentMonth ?? currentMonth) else {
                return dates
            }

            // Step 3: Ensure the day is set to 1 for the start of the next month
            guard let startOfNextMonth = calendar.date(bySetting: .day, value: 1, of: nextMonthStart) else { return dates }
            print("\(currentMonth) + \(nextMonthStart) + \(startOfNextMonth)")

            // Add the padding days from the next month
            for i in 0..<paddingDaysAfter {
                if let date = calendar.date(byAdding: .day, value: i, to: startOfNextMonth) {
                    dates.append(DateData(date: date, isCurrentMonth: false)) // Mark these as from the next month
                }
            }
        }
